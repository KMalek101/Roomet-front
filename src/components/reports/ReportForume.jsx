...
import { submitReport } from "@/utils/maintenance";

export default function ReportForum({ setShowReportForum, studentId, roomId }) {
  ...
  // Extract checked issues for submission
  ...
  const handleSubmit = async () => {
    ...
    if (selectedIssues.length === 0 && !description.trim()) {
      setError("Please select at least one issue or provide a description");
      return;
    }
    ...
    try {
      const reportData = {
        student: studentId,
        room: roomId,
        issues: description.trim() 
          ? [...selectedIssues, description] 
          : selectedIssues,
        urgency,
    ...
    <div className="flex flex-col gap-4 bg-[var(--secondary-color)] p-6 py-2 rounded-md flex-1">
      ...
      <div className="flex gap-1.5 ml-auto pt-4 pb-1 px-2">
        ...
        <button
          ...
          className="bg-[var(--green-color)] p-2 font-medium text-[var(--w-color)] rounded-md cursor-pointer disabled:opacity-50"
          disabled={isSubmitting}
        >
